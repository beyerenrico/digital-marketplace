...
import { AuthCredentialsValidator, VerifyEmailValidator } from '../lib/validations/authentication';
...
export const authRouter = router({
  ...
  verifyEmail: publicProcedure.input(VerifyEmailValidator).query(async ({ input }) => {
    const { token } = input;
    const payload = await getPayloadClient();
